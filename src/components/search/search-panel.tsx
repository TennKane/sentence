...
import { Loader2, Search, BookOpen, MessageSquare, Library } from "lucide-react";
...
function CombinedSearch() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      const data: CombinedSearchResponse = await res.json();
      setResults(data.matches);
      setMetadata({
        totalProcessed: data.totalProcessed,
        processingTimeMs: data.processingTimeMs,
    ...
    <div>
      ...
      {results !== null && !searching && (
        <>
          {results.length === 0 ? (
            <div className="py-16 text-center">
              <p className="text-ink-muted">没有找到匹配的内容</p>
              {metadata && (
                <p className="mt-2 text-xs text-ink-muted/60">
                  已检索 {metadata.totalProcessed} 条，用时 {(metadata.processingTimeMs / 1000).toFixed(1)}s
                </p>
            ...
            <>
              ...
              <div className="space-y-4">
                {results.map((match) => {
                  ...
                  if (match.type === "sentence") {
                    return (
                      <PaperCard key={`s-${match.id}`} elevation="raised">
                        <div className="mb-3 inline-block rounded-full bg-accent/10 px-3 py-1">
                          <span className="text-xs text-accent-dark font-medium">
                            {match.reason}
                          </span>
                  ...
                  return (
                    <PaperCard key={`a-${match.id}`} elevation="raised">
                      <div className="mb-3 inline-block rounded-full bg-accent/10 px-3 py-1">
                        <span className="text-xs text-accent-dark font-medium">
                          {match.reason}
                        </span>
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <BookOpen className="mt-1 h-5 w-5 shrink-0 text-accent/60" />
                          <div>
                            <h3 className="font-hand text-xl text-accent group-hover:underline">
                              {match.title}
                            </h3>
...
export function SearchPanel() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      {tab === "sentence" && <SentenceSearch />}
      {tab === "article" && <ArticleSearch />}
      {tab === "all" && <CombinedSearch />}
    </div>
  );
}
